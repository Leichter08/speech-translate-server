...
async def record_server_session(
    ...
    is_tc: bool,
    is_tl: bool,
    websocket_input: Optional[bytes] = None,
    speaker: bool = False,
) -> None:
    """
    ...
    try:
        ...
        while bc.recording:
            if paused:
                sleep(0.1)
                continue
            server_record_cb(websocket_input)
            ...
            # CHECKPOINT CHANGE TO WEBSOCKET BYTES
            # initialize stream
            # if using websocket input, the stream will be initialized with the input
            # data = BytesIO(websocket_input)
            # with w_open(data, 'rb') as wf:
